...
if($_GET['aksi'] == 'inputdivisi'){
    ...
    $input = $master->inputDivisi($dataDivisi);
    if($input){
        // Jika berhasil, redirect ke halaman master-prodi-list.php dengan status inputsuccess
        header("Location: ../master-divisi-list.php?status=inputsuccess");
    } else {
        // Jika gagal, redirect ke halaman master-prodi-input.php dengan status failed
        header("Location: ../master-divisi-input.php?status=failed");
    }
} elseif($_GET['aksi'] == 'updatedivisi'){
